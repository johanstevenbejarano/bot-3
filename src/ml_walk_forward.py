"""Walk-forward del clasificador sobre BTC/USDT: misma metodología que breakout/funding (16
folds, recalibra cada 90 días con los 365 previos), pero acá "recalibrar" es reentrenar el
modelo desde cero en cada fold, nunca con datos posteriores al corte de train.

Las features (miran hacia atrás) se calculan una sola vez sobre toda la serie — no hay riesgo de
fuga mirando hacia atrás. El labeling triple-barrera (mira hacia adelante hasta max_holding_bars)
se recalcula por separado dentro de cada tramo train/test: si se calculara una sola vez sobre la
serie completa y LUEGO se cortara en folds, las etiquetas de los últimos ~3 días de cada tramo de
train usarían precios que ya pertenecen al tramo de test siguiente — fuga de información.
"""
...
from src.ml_strategy import ModelBundle, RunResult, prepare_labeled, simulate, train_classifiers
...
def _grid_search_fn(train_slice: pd.DataFrame) -> ModelBundle | None:
    ...
    if labeled.empty:
        return None
    return train_classifiers(labeled, ML_STRATEGY)
...
def main() -> None:
    ...
    out_path = RESULTS_DIR / "ml_walk_forward_BTC.json"
    out_path.write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
    logger.info(
        "Walk-forward ML: %.2f%% acumulado sobre %d folds (%d con trades, %d positivos, %d trades totales)",
