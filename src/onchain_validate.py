"""Busca parámetros de la estrategia on-chain SOLO en train y valida en test — misma disciplina
que las líneas anteriores (ver FINDINGS.md): exige expectancy positiva en AMBOS pares a la vez en
train antes de mirar test.
"""
...
from src.config import BACKTEST, ONCHAIN_STRATEGY, PAIRS, RESULTS_DIR, OnchainConfig
from src.data_fetch import fetch_ohlcv
from src.onchain_indicators import add_onchain_indicators
from src.onchain_strategy import OnchainContrarianStrategy, compute_layers
...
def run_config(df_with_indicators: pd.DataFrame, cfg: OnchainConfig) -> RunResult:
    ...
    bt = FractionalBacktest(
        bt_data, OnchainContrarianStrategy,
...
def grid_search_best_config(
    train_raw_by_symbol: dict[str, pd.DataFrame],
...
) -> tuple[OnchainConfig | None, dict]:
    """Busca, SOLO en train, la config con mejor expectancy mínima consistente entre AMBOS
    símbolos -- misma disciplina que el resto de las líneas."""
    best_cfg = None
    ...
    for lookback, extreme_pct, sl_mult, tp_mult in product(
        ...
        per_symbol = {}
        for symbol, raw_train in train_raw_by_symbol.items():
            with_indicators = add_onchain_indicators(raw_train, symbol, cfg)
...
def main() -> None:
    ...
    if best_cfg is None:
        ...
        for symbol, raw_test in test_raw_by_symbol.items():
            with_indicators = add_onchain_indicators(raw_test, symbol, best_cfg)
    ...
    out_path = RESULTS_DIR / "onchain_train_test_validation.json"
