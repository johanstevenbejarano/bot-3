"""Busca parámetros de la estrategia de flujo institucional SOLO en train y valida en test —
misma disciplina que las líneas anteriores (ver FINDINGS.md): exige expectancy positiva en
AMBOS pares a la vez en train antes de mirar test.
"""
...
from src.config import BACKTEST, ORDERFLOW_STRATEGY, PAIRS, RESULTS_DIR, OrderflowConfig
from src.data_fetch import fetch_ohlcv
from src.orderflow_indicators import add_orderflow_indicators
from src.orderflow_strategy import OrderflowSqueezeStrategy, compute_layers

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s: %(message)s")
logger = logging.getLogger(__name__)

# Limitado por la retención real de open interest en Bybit (~4.9 años, ver FINDINGS.md
# "Línea 5") -- se deja margen para no pedir justo el límite exacto.
YEARS_OF_HISTORY = 4
TRAIN_FRACTION = 0.7
...
def run_config(df_with_indicators: pd.DataFrame, cfg: OrderflowConfig) -> RunResult:
    ...
    bt = FractionalBacktest(
        bt_data, OrderflowSqueezeStrategy,
...
def _train_test_slices(raw: pd.DataFrame, symbol: str, cfg: OrderflowConfig) -> tuple[pd.DataFrame, pd.DataFrame]:
    with_indicators = add_orderflow_indicators(raw, symbol, cfg, years=YEARS_OF_HISTORY)
    cutoff_idx = int(len(with_indicators) * TRAIN_FRACTION)
    return with_indicators.iloc[:cutoff_idx], with_indicators.iloc[cutoff_idx:]


def grid_search_best_config(
    train_by_symbol: dict[str, pd.DataFrame],
    oi_extreme_percentiles: tuple[float, ...] = (0.85, 0.90, 0.95),
    imbalance_extreme_zs: tuple[float, ...] = (1.0, 1.5, 2.0),
    sl_atr_mults: tuple[float, ...] = (2.0, 3.0, 4.0),
    tp_atr_mults: tuple[float, ...] = (4.0, 6.0, 8.0),
    min_trades_per_symbol: int = 20,
) -> tuple[OrderflowConfig | None, dict]:
    """Busca, SOLO en train, la config con mejor expectancy mínima consistente entre AMBOS
    símbolos (el peor caso, no el promedio) -- evita elegir una config que solo funciona en un
    par por azar, misma disciplina que `breakout_validate.grid_search_best_config`."""
    best_cfg = None
    best_score = float("-inf")
    best_detail: dict = {}

    for oi_pct, imb_z, sl_mult, tp_mult in product(
        oi_extreme_percentiles, imbalance_extreme_zs, sl_atr_mults, tp_atr_mults
    ):
        cfg = replace(
            ORDERFLOW_STRATEGY,
            oi_extreme_percentile=oi_pct, imbalance_extreme_z=imb_z,
...
def main() -> None:
    ...
    logger.info("Descargando/cacheando open interest (Bybit) y taker buy ratio (Binance)...")
    train_by_symbol = {}
    test_by_symbol = {}
    for symbol, raw in raw_by_symbol.items():
        train_df, test_df = _train_test_slices(raw, symbol, ORDERFLOW_STRATEGY)
    ...
    out_path = RESULTS_DIR / "orderflow_train_test_validation.json"
