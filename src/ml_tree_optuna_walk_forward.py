"""Walk-forward de Random Forest / HistGradientBoosting con búsqueda de hiperparámetros vía
Optuna + validación cruzada purgada, sobre 9 años de historia (train de 2 años por fold en vez
de 1) — la versión más rigurosa de la línea de clasificadores probada en la sesión.
"""
...
SYMBOL = "BTC/USDT"
YEARS_OF_HISTORY = 9  # BTC/USDT spot en Binance arranca en 2017-08
TRAIN_DAYS = 730  # 2 años por fold (vs. 1 año en los walk-forward anteriores) -- más datos por fit
TEST_DAYS = 90
STEP_DAYS = 180  # folds cada 6 meses (menos solapados, dado el train más largo)
N_TRIALS = 25
N_CV_SPLITS = 3
...
def main() -> None:
    ...
    report = {
        "symbol": SYMBOL,
        "strategy": "ml_tree_optuna (RF/HistGB, Optuna + CV purgada, train=2anios)",
    ...
    out_path = RESULTS_DIR / "ml_tree_optuna_walk_forward_BTC.json"
    out_path.write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
    logger.info(
        "Walk-forward RF/HGB+Optuna: %.2f%% acumulado sobre %d folds (%d con trades, %d positivos, %d trades totales)",
